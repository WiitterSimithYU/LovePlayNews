    ...
    [posts enumerateObjectsUsingBlock:^(LPDiscuzPost *post, NSUInteger idx, BOOL * stop) {
        if (idx > 0) {
            ...
            if (rangeStart.length > 0 && rangeEnd.length > 0 && rangeStart.location < rangeEnd.location) {
                NSString *postText = [post.message substringWithRange:NSMakeRange(rangeStart.location, rangeEnd.location+rangeEnd.length-rangeStart.location)];
                post.replyText = postText;
                NSLog(@"+++______%@",post.message);
                NSLog(@"____________>>>%@",postText);
